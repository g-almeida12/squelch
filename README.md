...
*   **`challenges.config.ts`**: The main script responsible for loading, creating, and seeding all challenges into the application's primary database.
*   **`groups/`**: A folder containing subdirectories for each individual mystery (e.g., `furto-noturno`, `o-caso-do-auditor`). Each mystery's folder contains:
    ...
    *   **`challenge.db`**: The isolated SQLite database file containing all the clues and tables for that group.
...
##### 1. `config`
This folder handles the environment setup. It manages the physical creation of database files on disk, loads environment variables, and hosts a **Dependency Injection Factory** that instantiates and exports controllers, services, and repositories.
...
Run the shared script to compile the shared code used in both server and client applications:
```bash
npm run shared:build -w @squelch/shared
```

5. **start the server**
Run the server script to generate all the SQLite databases for the challenges and start the server:
```bash
npm run server:dev -w server
```

6. **start the client**
Finally, run the client server to start the frontend application at http://localhost:5173:
```bash
npm run client:dev -w client
```